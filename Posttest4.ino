...
#define WIFI_SSID "Xiaomi 13T"
#define WIFI_PASSWORD "Dhiya123"


// Initialize Telegram BOT
#define BOTtoken "7087814442:AAFV4TDeATKoF-B8K9t-W-PnKa8xc0IfQ-0"  // your Bot Token (Get from Botfather)
#define DHTPIN D6
#define LED1 D1
#define LED2 D2
#define LED3 D3
...
String user1ControlledLight = "1197170312";
String user2ControlledLight = "1400989657";
String user3ControlledLight = "1164302353";
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/LED1on" || text == "/LED1on@Posstest4_Bot") {
    ...
    if (text == "/LED1off" || text == "/LED1off@Posstest4_Bot") {
      String from_name = bot.messages[i].from_id;
      if (from_name == user1ControlledLight){
    ...
    if (text == "/LED2on" || text == "/LED2on@Posstest4_Bot") {
      String from_name = bot.messages[i].from_id;
    ...
    if (text == "/LED2off" || text == "/LED2off@Posstest4_Bot" ) {
      String from_name = bot.messages[i].from_id;
      if (from_name == user2ControlledLight){
    ...
    if (text == "/LED3on" || text == "/LED3on@Posstest4_Bot") {
      String from_name = bot.messages[i].from_id;
    ...
    if (text == "/LED3off" || text == "/LED3off@Posstest4_Bot") {
      String from_name = bot.messages[i].from_id;
      if (from_name == user3ControlledLight){
    ...
    if (text == "/LED4on" || text =="/LED4on@Posstest4_Bot") {
      digitalWrite(LED4, HIGH);
      led4Status = 1;
      bot.sendMessage(chat_id, "Led is ON", "");
    }

    if (text == "/LED4off" || text == "/LED4off@Posstest4_Bot") {
      led4Status = 0;
      digitalWrite(LED4, LOW); // turn the LED off (LOW is the voltage level)
      bot.sendMessage(chat_id, "Led is OFF", "");
    }

    if (text == "/ledstatus" || text == "/ledstatus@Posstest4_Bot") {
      String led1;
      String led2;
      String led3;
      String led4;
    ...
    if (text == "/dhtstatus" || text == "/ledstatus@Posstest4_Bot") {
      String message = "";
